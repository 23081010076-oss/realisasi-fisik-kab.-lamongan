...
const formatRupiah = (number) => {
  ...
  if (number >= 1_000_000_000)
    return `Rp ${(number / 1_000_000_000).toFixed(2)} M`;
...
export default function Dashboard() {
  ...
  const paketKritis = recentUpdates.filter(
    (p) => p.status === "ACTIVE" && p.progres < 30,
  );
  ...
  const topOpd = (chartData?.opd || [])
    ...
    .sort(
      (a, b) =>
        b.totalRealisasi / b.totalNilai - a.totalRealisasi / a.totalNilai,
    )
    .slice(0, 3);
    ...
    datasets: [
      {
        ...
        data: monthNames.map(
          (_, idx) =>
            chartData?.monthly?.find((m) => m.month === idx + 1)?.count || 0,
        ),
        borderColor: "#2563eb",
      ...
      },
      {
        ...
        data: monthNames.map(
          (_, idx) =>
            chartData?.monthly
              ?.find((m) => m.month === idx + 1)
              ?.avgProgres?.toFixed(1) || 0,
        ),
        borderColor: "#059669",
  ...
  const opdBarData = {
    labels: (chartData?.opd || [])
      .slice(0, 8)
      .map((o) => o.opd?.code || o.opd?.name?.slice(0, 10) || ""),
    datasets: [
      {
        label: "Nilai Kontrak",
      ...
      <div className="bg-gradient-to-r from-[#0a2240] via-[#0d3060] to-[#0a2240] text-white px-4 sm:px-6 py-3 rounded-t-2xl flex items-center justify-between gap-3">
        <div className="flex items-center min-w-0 gap-3">
          <img
            ...
            alt="Lamongan"
            className="object-cover w-10 h-10 rounded-full shrink-0 opacity-90"
            onError={(e) => {
              e.target.style.display = "none";
            }}
          />
          <div className="min-w-0">
            <p className="text-[11px] font-semibold tracking-widest text-blue-300 uppercase">
      ...
      <div className="pt-4 space-y-4">
          ...
          <div className="p-5 bg-white border shadow-sm lg:col-span-5 rounded-2xl dark:bg-gray-900">
            ...
            <div className="relative mt-2 mb-4">
              ...
              <div className="flex justify-between mt-1 px-0.5">
                {[25, 50, 75, 100].map((m) => (
                  ...
                  >
                    <span
                      className={`text-[10px] ${pct >= m ? "text-blue-600 font-bold" : "text-gray-400"}`}
                    >
                      {m}%
                    </span>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 border border-gray-100 rounded-xl bg-gray-50 dark:bg-gray-800 dark:border-gray-700">
                ...
                <p className="text-sm font-black leading-tight text-gray-900 dark:text-white">
                  {formatRupiahFull(stats?.nilai?.total)}
                </p>
              </div>
              <div className="p-3 border rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border-emerald-100 dark:border-emerald-800">
                ...
                <p className="text-sm font-black leading-tight text-emerald-700">
                  {formatRupiahFull(stats?.nilai?.realisasi)}
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-400">
                    Dari seluruh paket aktif
                  </p>
                </div>
              </div>
              <p className="text-2xl font-black text-blue-700">
                {stats?.progres?.average || 0}%
              </p>
            </div>
          </div>
            ...
            {topOpd.length > 0 ? (
              <div className="space-y-3">
                {topOpd.map((o, idx) => {
                  const realisasiPct =
                    o.totalNilai > 0
                      ? Math.round((o.totalRealisasi / o.totalNilai) * 100)
                      : 0;
                  const medals = ["", "", ""];
                  return (
                    <div key={idx} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-0.5">
                          <p className="text-xs font-semibold text-gray-800 truncate dark:text-gray-200">
                            {o.opd?.code || o.opd?.name?.slice(0, 20) || ""}
                          </p>
                          <span
                            className={`text-xs font-bold ml-2 shrink-0 ${realisasiPct >= 80 ? "text-emerald-600" : realisasiPct >= 50 ? "text-blue-600" : "text-amber-600"}`}
                          >
                            {realisasiPct}%
                          </span>
                        </div>
                        <div className="h-1.5 w-full rounded-full bg-gray-100 dark:bg-gray-700 overflow-hidden">
                          <div
                            className={`h-full rounded-full ${realisasiPct >= 80 ? "bg-emerald-500" : realisasiPct >= 50 ? "bg-blue-500" : "bg-amber-500"} transition-all duration-700`}
                            style={{ width: `${Math.min(realisasiPct, 100)}%` }}
                          />
                        </div>
                        <p className="text-[10px] text-gray-400 mt-0.5">
                          {formatRupiah(o.totalRealisasi)} /{" "}
                          {formatRupiah(o.totalNilai)}
                        </p>
                      </div>
                    </div>
                  );
        ...
        <div className="grid gap-4 grid-cols-1 lg:grid-cols-12">
          ...
          <div className="p-5 bg-white border shadow-sm lg:col-span-7 rounded-2xl dark:bg-gray-900">
            ...
            <p className="mb-4 text-xs text-gray-400">
              Perbandingan progres seluruh status kegiatan pengadaan TA {tahun}
            </p>
                  ...
                  <div key={label} className="flex items-center gap-1.5">
                    ...
                    {totalPaket > 0 && (
                      <span className="text-[10px] text-gray-400">
                        ({Math.round((count / totalPaket) * 100)}%)
                      </span>
                    )}
                  </div>
                ))}
          ...
          <div className="flex flex-col p-5 bg-white border shadow-sm lg:col-span-5 rounded-2xl dark:bg-gray-900">
            <div className="flex items-center gap-2 mb-3">
              <div
                className={`rounded-lg p-1.5 ${paketKritis.length > 0 ? "bg-red-100 dark:bg-red-900/40" : "bg-emerald-100 dark:bg-emerald-900/40"}`}
              >
              ...
              <div>
                <p className="text-sm font-bold text-gray-900 dark:text-white">
                  {paketKritis.length > 0
                    ? `${paketKritis.length} Paket Perlu Perhatian`
                    : "Semua Paket On-Track"}
                </p>
                <p className="text-xs text-gray-400">
                  Paket aktif dengan progres &lt; 30%
                </p>
              </div>
            </div>
            ...
            {paketKritis.length > 0 ? (
              <div className="flex-1 space-y-2 overflow-y-auto max-h-52">
                {paketKritis.map((p) => (
                  <div
                    key={p.id}
                    className="flex items-start gap-2 rounded-xl bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-800/50 p-2.5"
                  >
              ...
              <div className="flex flex-col items-center justify-center flex-1 py-4 text-center">
                ...
                <p className="mt-1 text-xs text-gray-400">
                  Tidak ada paket aktif dengan progres di bawah 30%
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-2 pt-3 mt-3 border-t border-gray-100 dark:border-gray-800">
              <div className="text-center">
                <p className="text-lg font-black text-gray-900 dark:text-white">
                  {totalPaket > 0
                    ? Math.round((totalSelesai / totalPaket) * 100)
                    : 0}
                  %
                </p>
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-sm font-bold text-gray-800 dark:text-white">
              Rekapitulasi Per Jenis Pengadaan
            </p>
          </div>
            ...
            {[
              {
                ...
                value: stats?.kategori?.konstruksi || 0,
                icon: Building2,
                from: "from-blue-600",
                to: "to-blue-700",
                ring: "ring-blue-700/40",
                sub: "Infrastruktur & bangunan",
              },
              {
                label: "Jasa Konsultansi",
                value: stats?.kategori?.konsultansi || 0,
                icon: Briefcase,
              ...
              },
              {
                ...
                value: stats?.kategori?.barang || 0,
                icon: Package,
                from: "from-orange-500",
                to: "to-amber-600",
                ring: "ring-orange-600/40",
                sub: "Alat, bahan & material",
              },
              {
                label: "Jasa Lainnya",
                value: stats?.kategori?.jasaLainnya || 0,
                icon: Wrench,
                from: "from-violet-600",
                to: "to-purple-700",
                ring: "ring-violet-700/40",
                sub: "Pemeliharaan & layanan",
              },
            ].map(({ label, value, icon: Icon, from, to, ring, sub }) => {
              const share = totalPaket
                ? Math.round((value / totalPaket) * 100)
                : 0;
              return (
                <div
                  key={label}
                  className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${from} ${to} p-4 text-white shadow-md ring-2 ${ring} hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200`}
                >
                  ...
                  <div className="mt-3 h-1.5 w-full rounded-full bg-white/20 overflow-hidden">
                    <div
                      className="h-full transition-all duration-700 rounded-full bg-white/60"
                      style={{ width: `${share}%` }}
                    />
                  </div>
                </div>
        ...
        <div className="grid gap-4 grid-cols-1 lg:grid-cols-7">
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  Jumlah paket & rata-rata progres per bulan TA {tahun}
                </p>
              </div>
          ...
          <div className="p-5 bg-white border shadow-sm lg:col-span-3 rounded-2xl dark:bg-gray-900">
            <div className="mb-4">
              <p className="text-sm font-bold text-gray-900 dark:text-white">
                Komposisi Jenis Pengadaan
              </p>
              <p className="text-xs text-gray-400 mt-0.5">
                Proporsi berdasarkan jumlah paket TA {tahun}
              </p>
            </div>
                ...
                options={{
                  ...
                  plugins: {
                    ...
                    tooltip: {
                      callbacks: {
                        label: (ctx) =>
                          ` ${ctx.label}: ${ctx.raw} paket (${totalPaket ? Math.round((ctx.raw / totalPaket) * 100) : 0}%)`,
                      },
                    },
                  },
                }}
        ...
        {(chartData?.opd?.length || 0) > 0 && (
          <div className="p-5 bg-white border shadow-sm rounded-2xl dark:bg-gray-900">
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="text-sm font-bold text-gray-900 dark:text-white">
                  Nilai Kontrak vs Realisasi per OPD
                </p>
                <p className="text-xs text-gray-400 mt-0.5">
                  8 OPD dengan nilai kontrak terbesar TA {tahun}
                </p>
              </div>
              <div className="flex gap-3 text-[10px] text-gray-400">
                <div className="flex items-center gap-1">
                  <div className="w-4 h-2 bg-blue-500 rounded-sm opacity-80" />
                  Kontrak
                </div>
                <div className="flex items-center gap-1">
                  <div className="w-4 h-2 rounded-sm bg-emerald-500 opacity-80" />
                  Realisasi
                </div>
              </div>
            </div>
                ...
                options={{
                  ...
                  plugins: {
                    ...
                    tooltip: {
                      ...
                      callbacks: {
                        label: (ctx) =>
                          ` ${ctx.dataset.label}: ${formatRupiahFull(ctx.raw)}`,
                      },
                    },
                  },
                  scales: {
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider w-8">
                    #
                  </th>
                  ...
                  <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider hidden md:table-cell">
                    Jenis
                  </th>
                  ...
                  <th className="px-4 py-2.5 text-center text-[11px] font-semibold text-gray-400 uppercase tracking-wider hidden sm:table-cell">
                    Status
                  </th>
                  <th className="px-4 py-2.5 text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wider hidden lg:table-cell">
                    Diperbarui
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                {recentUpdates.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-xs font-bold text-blue-700 dark:text-blue-400 whitespace-nowrap">
                      {item.opd?.code || ""}
                    </td>
                    ...
                    <td className="hidden px-4 py-3 md:table-cell">
                      <span className="text-[11px] text-gray-500 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full">
                        {item.kategori}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-mono text-xs font-semibold text-right text-gray-900 dark:text-white whitespace-nowrap">
                      {formatRupiah(item.nilai)}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <div className="flex flex-col items-center gap-1">
                        <span
                          className={`text-xs font-black ${progresTextColor(item.progres)}`}
                        >
                          {item.progres}%
                        </span>
                        <div className="h-2 overflow-hidden bg-gray-100 rounded-full w-14 dark:bg-gray-700">
                          <div
                            className={`h-full rounded-full ${progresColor(item.progres)} transition-all duration-500`}
                    ...
                    <td className="px-4 py-3 text-right text-[11px] text-gray-400 whitespace-nowrap hidden lg:table-cell">
                  ...
                  <tr>
                    <td colSpan={8} className="px-4 py-16 text-center">
                      <div className="flex flex-col items-center gap-2 text-gray-300">
                        ...
                        <p className="text-sm">
                          Belum ada data paket untuk tahun {tahun}
                        </p>
                      </div>
                    </td>
                  </tr>
            ...
            <div className="px-5 py-3 border-t bg-gray-50 dark:bg-gray-800/50 text-[11px] text-gray-400 flex items-center justify-between">
              ...
              <span className="flex items-center gap-2">
                <span className="inline-flex items-center gap-1">
                  <div className="w-2 h-2 bg-red-400 rounded-full" /> Progres
                  rendah
                </span>
                <span className="inline-flex items-center gap-1">
